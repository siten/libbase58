...
typedef unsigned long long b58_maxint_t;
typedef unsigned int b58_almostmaxint_t;
...
bool b58tobin(void *bin, size_t *binszp, const char *b58, size_t b58sz)
{
    ...
    size_t i, j;
    unsigned char bytesleft = binsz % sizeof(b58_almostmaxint_t);
...
int b58check(const void *bin, size_t binsz, const char *base58str, size_t b58sz)
{
    ...
    const unsigned char *binc = (unsigned char *)bin;
    unsigned i;
    if (binsz < 4)
        return -4;
...
{
    bool bret = true;
    unsigned char *buf = (unsigned char *)malloc(sizeof(unsigned char) * (1 + datasz + 0x20));
    if (nullptr == buf) {
        return false;
    }
    unsigned char *hash = &buf[1 + datasz];
    ...
    bret = b58enc(b58c, b58c_sz, buf, 1 + datasz + 4);
__exit:
    free(buf);
    return bret;
}